...
- **`reminder_minutes`/`reminder_method` exist in the code and config schema but do nothing visible to the real calendar owner** - same root cause as the attendees limitation. Google's reminders are private per authenticated identity; the Lambda authenticates as the service account, so any reminder override it sets is only visible to the service account, never to the human who actually owns the calendar. This was implemented, deployed, and only discovered to be non-functional after live testing (events kept showing `updated` daily and reminders never appeared in the actual calendar) - don't re-implement per-event reminders without first confirming the auth model has changed (e.g. real OAuth as the user, not a service account). The functional alternative is calendar-level default reminders, set by the owner via Google Calendar's own Settings UI - see README section 2.
